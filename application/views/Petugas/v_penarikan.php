            ...
            <div class="panel-heading" role="tab" id="headingOne">
                <h4 class="panel-title">
                   <a class="btn btn-xs default" href="javascript:;" id="form_act">
                    ...
                    <input type="hidden" name="id_penarikan" id="id_penarikan" value=""/>
                                         <div class="table-responsive">
                                        <table class="table table-striped table-bordered table-hover table-checkable order-column" id="sample_1">

                                                <tr>
                                                   <td colspan="6" style="font-weight:bold"> Detail Penarikan Saldo </td>
                                                 ...
                                                 <tr>
                                                    ...
                                                    <td width="10px"> : </td>
                                                     <td colspan="6">  
                                                      <select name="id_nasabah" id="id_nasabah" class="form-control" onchange="ganti_sisa()">
                                                 ...
                                                 <tr>
                                                     <td width="150px"> Sisa Saldo  </td>
                                                    <td width="10px"> : </td>
                                                    <td>  
                                                     <input type="number" class="form-control" name="sisa_saldo" id="sisa_saldo" tabindex="1" readonly>
                                                     </td>
                                                    <td width="150px"> Jumlah Penarikan  </td>
                                                    <td width="10px"> : </td>
                                                    <td>  
                                                     <input type="number" class="form-control" name="jumlah_penarikan" id="jumlah_penarikan" tabindex="1">
                                                     </td>
  ...
  var initTable1 = function() {
    ...
    */
    ...
    var oTable = $(target).dataTable( {
        ...
        "sAjaxSource": "<?php echo base_url(); ?>c_penarikan/view", // define ajax source URL
        "sServerMethod": "POST"
      });
    ...
    $(document).ready(function() {
      ...
      $('#save').click(function(){
       if ($('#id_nasabah').val() == "") {
          ...
          $('#id_nasabah').focus();
          return false;
        }else if ($('#jumlah_penarikan').val() == "") {
          alert("Jumlah Penarikan harus di isi");
          $('#jumlah_penarikan').focus();
          return false;
        }else if ($('#sisa_saldo').val() < $('#jumlah_penarikan').val()) {
          alert("Sisa saldo kurang untuk ditarik");
          $('#jumlah_penarikan').focus();
          return false;
        }else {
...
function set_val(data) {
  // body...
   ...
   $.post('<?php echo base_url()?>c_penarikan/dataedit', {id_penarikan: data}, function(respond) {  
                  
                  $("#id_penarikan").val(respond[0].id_penarikan);
                 
                  
                
                  $("#id_nasabah").val(respond[0].id_nasabah);
                  $("#jumlah_penarikan").val(respond[0].jumlah_penarikan);
...
function set_del(data) {
  if( confirm("Apakah data ingin dihapus ?") ){
    ...
    $.ajax({
                type: 'POST',
                url: '<?php echo base_url() ?>c_penarikan/delete',
                data: { id : data},
                success: function(data) {
                    if(data === 'true')
                    // $('#cancel').click();
...
function ganti_sisa() {
   ...
   $.ajax({
                type: 'POST',
                url: '<?php echo base_url() ?>c_penarikan/ganti_sisa',
                data: { id : data},
                success: function(data) {
                    $('#sisa_saldo').val(data);
                }
