            ...
            <div class="panel-heading" role="tab" id="headingOne">
                <h4 class="panel-title">
                   <a class="btn btn-xs default" href="javascript:;" id="form_act">
                    ...
                    <input type="hidden" name="id_jenis_sampah" id="id_jenis_sampah" value=""/>
                                         <div class="table-responsive">
                                        <table class="table table-striped table-bordered table-hover table-checkable order-column" id="sample_1">

                                                <tr>
                                                   <td colspan="6" style="font-weight:bold"> Detail Jenis Sampah </td>
                                                 ...
                                                 <tr>
                                                     ...
                                                     <td width="150px"> Nama Jenis Sampah  </td>
                                                    <td width="10px"> : </td>
                                                    <td>  <input type="text" class="form-control" name="nama_sampah" id="nama_sampah" tabindex="1"> </td>
                                                     <td width="150px"> Harga Per KG </td>
                                                    <td width="10px"> : </td>
                                                    <td>  <input type="text" class="form-control" name="harga_per_kg" id="harga_per_kg" tabindex="1"> </td>
                                                </tr> 
...
<div class="row">
  ...
  var initTable1 = function() {
    ...
    */
    ...
    var oTable = $(target).dataTable( {
        ...
        "sAjaxSource": "<?php echo base_url(); ?>c_sampah/view", // define ajax source URL
        "sServerMethod": "POST"
      });
  ...
  return {
    ...
    $(document).ready(function() {
      ...
      $('#save').click(function(){
       if ($('#nama_sampah').val() == "") {
          alert("Nama Sampah harus di isi");
          $('#nama_sampah').focus();
          return false;
        }else if ($('#harga_per_kg').val() == "") {
          alert("Harga Per KG harus di isi");
          $('#harga_per_kg').focus();
          return false;
        }else {
...
function set_val(data) {
  // body...
   ...
   $.post('<?php echo base_url()?>c_sampah/dataedit', {id_jenis_sampah: data}, function(respond) {  
                  
                  $("#id_jenis_sampah").val(respond[0].id_jenis_sampah);
                 
                  $("#nama_sampah").val(respond[0].nama_sampah);
                
                  $("#harga_per_kg").val(respond[0].harga_per_kg);
...
function set_del(data) {
  if( confirm("Apakah data ingin dihapus ?") ){
    $.ajax({
                type: 'POST',
                url: '<?php echo base_url() ?>c_sampah/delete',
                data: { id : data },
                success: function(data) {
                    if(data === 'true')
                    // $('#cancel').click();
